...
Begin VB.Form FrmZZLFX 
   ...
   Begin UserControl.lst lst1 
      ...
      BackColorSel    =   49152
      RowHeightMin    =   300
      ColumnCaptions  =   "内容;接收时间;导入时间;接导时间;售后处理时间;导售数;转研发时间;研发解决时间;研解数;售后测试时间;售研数"
      ColWidths       =   ""
      ColumnName      =   "内容;接收时间;导入时间;接导时间;售后处理时间;导售数;转研发时间;研发解决时间;研解数;售后测试时间;售研数"
   End
...
Private Sub cmd1_Click(Index As Integer)
    ...
    Case 0
        sWhere = ""
        If cbo1.Text <> "" Then
            sWhere = sWhere & " and 售后人员='" & cbo1.Text & "'"
        End If
        If DTPicker1(0).CheckBox Then
            sWhere = sWhere & " and 导入时间>='" & DTPicker1(0).Value & "'"
        End If
        If DTPicker1(1).CheckBox Then
            sWhere = sWhere & " and 导入时间<='" & DTPicker1(1).Value & "'"
        End If
        objFormSetting.FillList Me.lst1, "select * from st_yw_wentilb where 1=1   " & sWhere
    Case 2
    End Select
End Sub
