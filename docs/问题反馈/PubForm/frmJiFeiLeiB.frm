...
Begin VB.Form frmJiFeiLeiB 
   ...
   Begin UserControl.lst lstData 
      ...
      ColWidths       =   $"frmJiFeiLeiB.frx":0000
   ...
   End
End
Attribute VB_Name = "frmJiFeiLeiB"
...
Private Sub cmdShanChu_Click()
   ...
   If IsAllowDeleted("zd_JiFeiLeiBie", lstData.cellValue(lstData.getSelectedRow, "DaiMa")) = False Then
...
Private Function ShanChuSJ() As Boolean
   ...
   If objExecuteSQL.DeleteRecord("zd_JiFeiLeiBie", g_strTiaoJian) = False Then
      ShanChuSJ = False
      GoTo PROC_EXIT
   End If
...
Private Sub cmdZengJia_Click()
   ...
   If txtDaiMa.Text = "" Then
      txtDaiMa.Text = getDaiMa("zd_JiFeiLeiBie", "DaiMa")
   End If
   ...
   If objDataCheck.IsRepeated("zd_JiFeiLeiBie", g_strTiaoJian, Me.Name, g_eTiShiXLH.ShuJvKSJCF) = False _
...
Private Function ZengJiaSJ() As Boolean
   ...
   '如果没有执行成功SQL语句
   If objExecuteSQL.InsertRecord("zd_JiFeiLeiBie", colInsert) = False Then
      ZengJiaSJ = False
      GoTo PROC_EXIT
   End If
...
Private Function XiuGaiSJ() As Boolean
   ...
   '如果没有执行成功SQL语句
   g_strTiaoJian = "DaiMa =  '" & lstData.cellValue(m_lngRow, "DaiMa") & "'"
   If objExecuteSQL.UpdateRecord("zd_JiFeiLeiBie", g_strTiaoJian, colUpdate) = False Then
      XiuGaiSJ = False
      GoTo PROC_EXIT
   End If
...
Private Function CreateSql() As String
   ...
   '转化标准SQL语句
   CreateSql = objExecuteSQL.CreateSelectSQL("zd_JiFeiLeiBie", colSearch, g_strTiaoJian)
